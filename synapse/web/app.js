...
function capabilityRecord(agent,profile){return capabilityData.find(x=>x.agent===agent&&x.profile===profile)||null}
function agentProfiles(a){const caps=a.client?.profile_capabilities||{};return capabilityData.filter(x=>x.agent===a.name&&x.profile).map(x=>({...caps[x.profile],...x.declared,name:x.profile,capability_id:x.id,revision:x.revision,suggested_timeout:x.timeout_hint}))}
...
function profileHints(p){const hints=Array.isArray(p.hints)?p.hints:[];const out=[];if(p.suggested_timeout||p.timeout)out.push(`timeout ${p.suggested_timeout||p.timeout}s`);if(p.max_output_bytes)out.push(`max ${p.max_output_bytes}B`);if(p.supports_session)out.push('session');if(p.session_required)out.push('requires session');if(hints.includes('avoid_short_timeout'))out.push('avoid 60s');if(p.advisory_safe)out.push('read-only advisory');for(const tag of (Array.isArray(p.tags)?p.tags:[]))out.push(tag);return [...new Set(out)]}
function renderProfileRow(a,p){const aliases=Array.isArray(p.session_aliases)?p.session_aliases:[];const aliasText=p.supports_session?(aliases.length?`aliases: ${aliases.join(', ')}`:(p.allow_raw_session_id?'raw session id allowed':'session configured by local worker')):'no session';const pills=profileHints(p).map(x=>`<span class="pill">${e(x)}</span>`).join('');const payload=JSON.stringify(agentCallPayload(a.name,p),null,2);return `<div class="profile-row"><div><strong>${e(p.name)}</strong><div class="pills">${pills}</div><div class="muted">${e(aliasText)} · plan ${e(p.plan_delivery||'argv')}</div><div class="muted">${e(p.capability_id||'')} · ${e(p.revision||'')}</div></div><div class="task-actions">${copyButton(payload)}</div></div>`}
...
async function ragents(){const tags=await api('GET','/admin/capabilities');const d=tags.agents||{};capabilityData=Array.isArray(tags.capabilities)?tags.capabilities:[];const agents=Array.isArray(d.available)?d.available:[];const online=agents.filter(x=>x.online).length;const items=agents.map(renderAgentItem).join('')||'<div class="empty">暂无 Agent</div>';return `<div class="list"><h3>能力目录</h3><div class="grid"><div class="stat"><strong>${online}/${agents.length}</strong><span>在线 Agent</span></div><div class="stat"><strong>${Array.isArray(d.configured)?d.configured.length:0}</strong><span>配置项</span></div><div class="stat"><strong>${Array.isArray(d.online)?d.online.length:0}</strong><span>WebSocket</span></div></div>${renderProbeResults()}${items}<h3 class="section-title">原始数据</h3><pre id="agents-json" class="inline-code raw-json">${e(JSON.stringify(d,null,2))}</pre></div><div class="form"><div class="card"><h3>连接检查</h3><button class="btn" data-action="probe-agents">广播探测</button><button class="btn secondary" data-action="reload">刷新</button><button class="btn secondary" data-action="copy-target" data-target="agents-json">复制原始 JSON</button></div></div>`}
...
function taskEndpoints(advisoryOnly=false){return capabilityData.filter(x=>x.dispatchable&&(!advisoryOnly||x.declared.advisory_safe)).map(x=>({agent:x.agent,profile:x.profile,label:x.id,advisory:!!x.declared.advisory_safe,timeout:x.timeout_hint}))}
...
async function rtasks(){const [taskData,groupData,catalogData,statData]=await Promise.all([api('GET','/admin/tasks?limit=100'),api('GET','/admin/task-groups?limit=30'),api('GET','/admin/capabilities'),api('GET','/admin/stats/agents?days=30')]);capabilityData=Array.isArray(catalogData.capabilities)?catalogData.capabilities:[];const tasks=Array.isArray(taskData.tasks)?taskData.tasks:[];const groups=Array.isArray(groupData.groups)?groupData.groups:[];const direct=tasks.filter(x=>!x.group_id);return `<div class="list"><h3>Agent 调用统计（30 天）</h3>${renderAgentStats(statData.stats||[])}${renderTaskDetail()}<h3 class="section-title">任务组</h3>${groups.map(renderGroup).join('')||'<div class="empty">暂无任务组</div>'}<h3 class="section-title">直接任务</h3>${direct.map(t=>renderTask(t)).join('')||'<div class="empty">暂无直接任务</div>'}</div><div class="form"><div class="card"><h3>实时状态</h3><p class="muted">任务流：<span id="task-stream-state" class="badge warn">连接中</span></p><button class="btn secondary" data-action="reload">刷新</button></div>${taskComposer()}</div>`}
